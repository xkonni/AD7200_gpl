					...
					enum nss_qdisc_bshaper_tasks task)
{
	...
	while(dev) {
		 ...
		 */
		if (is_vlan_dev(dev)) {
			nss_if_num = nss_cmn_get_interface_number(nq->nss_shaping_ctx, vlan_dev_priv(dev)->real_dev);
		}
		else
		{
		...
		for (j=0; j < br_update.port_list_count; j++) {
			if (br_update.port_list[j] == nss_if_num)
				goto nextdev;
		}
