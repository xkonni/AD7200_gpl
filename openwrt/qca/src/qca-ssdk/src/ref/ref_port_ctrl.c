...
qca_ar8327_sw_mac_polling_task(struct switch_dev *dev)
{
	...
	a_uint32_t i;
	a_uint32_t reg, value;
	a_uint32_t link, speed, duplex, mdi, autoNeg;
	a_uint32_t phy_addr;
	a_uint32_t phy_reg;
	a_uint16_t phy_val;
	a_uint32_t qm_val;
	...
	a_uint16_t phy_ctrl[AR8327_NUM_PORTS];
	...
	for (i = 1; i < AR8327_NUM_PORTS-1; ++i) {
		...
		qca_ar8327_phy_read(0, i-1, F1_PHY_CONTROL, &phy_ctrl[i]);
		autoNeg = (a_uint32_t)((phy_ctrl[i] & BIT(12)) >> 12);
		if(0 == autoNeg){
			speed = (a_uint32_t)((phy_ctrl[i] >> 13) & 0x03);
			duplex = (a_uint32_t)((phy_ctrl[i] & BIT(8)) >> 8);
		}
					...
					if ((port_old_speed[i] == AR8327_PORT_SPEED_100M)
						...
						if(0 == autoNeg){
							...
							case AR8327_PORT_SPEED_100M:
								...
								break;
							default:
								phy_val = F1_CTRL_SPEED_1000;
								break;
							}
							if(duplex == 1){
								phy_val |= F1_CTRL_FULL_DUPLEX;
							}
							phy_val |= F1_CTRL_SOFTWARE_RESET;
							qca_phy_mutex_write(0, phy_addr, phy_reg, data_mask, &phy_val);
						}
					}
					else {
