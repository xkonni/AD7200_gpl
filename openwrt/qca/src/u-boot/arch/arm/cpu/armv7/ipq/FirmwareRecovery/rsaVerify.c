 ...
 */
static int rsaVerifySignByBase64EncodePublicKeyBlob(unsigned char *pPublicKeyBlob,
 						unsigned long PublicKeyBlobLen,
 						unsigned char *pInData,      
 						unsigned long inDataLen,		   
 						unsigned char *PInSignature,   
...
int check_cloud(unsigned char *buf, int len)
{
    ...
    md5_make_digigned(md5_dig, buf + IMAGE_SIZE_FWTYPE, len - IMAGE_SIZE_FWTYPE);

    ret = rsaVerifySignByBase64EncodePublicKeyBlob(rsaPubKey, strlen((char *)rsaPubKey),
            md5_dig, IMAGE_SIZE_MD5, tmp_rsa_sig, IMAGE_LEN_RSA_SIG);
