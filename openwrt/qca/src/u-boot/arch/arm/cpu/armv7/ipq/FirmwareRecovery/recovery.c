...
static int check_supportlist(const unsigned char *buf)
{
	...
	{
		argc = rec_str_split((unsigned char*)argv_list[i], strlen(argv_list[i]) + 1, ":, ", argv, PRODUCT_INFO_MAX_NUM * 2);
		if (argc == 0)
		{
			goto err_out1;
		}
...
static int check_verify(unsigned char *buf, int len)
{
    unsigned char md5[MD5_LEN];
    int ret = 0;
    char fw_type_name[64] = {0};
    unsigned char *pfw;
    int i;

    pfw = buf + IMAGE_SIZE_FWTYPE;

    if(memcmp(pfw, "fw-type:", 8) == 0)
    {
        i = 0;
        pfw = pfw + 8;
        while(pfw[i] != '\"' && pfw[i] != '\t' && pfw[i] != '\r' && pfw[i] != '\n')
            i++;
        memcpy(fw_type_name, pfw, i);
    }
    REC_DEBUG("fw-type:\"%s\"", fw_type_name);
    ...
    /*
    else if(strcmp("some_other_type") == 0)
    {
        ret = check_other_type();
    }*/
    else
    {
        //check md5
