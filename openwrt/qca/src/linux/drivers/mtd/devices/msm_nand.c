...
struct mtd_info *nand_mtd_for_flash_chardev;
...
int msm_nand_scan(struct mtd_info *mtd, int maxchips)
{
	...
	chip->ecc_buf_cfg = boot_layout ? 0x1ff : 0x203;
	...
	if (enable_bch_ecc) {
		...
		chip->ecc_bch_cfg = (0 << 0) /* Enable ECC*/
			...
			|   ((boot_layout? 512 : 516) << 16) /* 516 user data bytes */
                                   ...
                                   const char *buf, size_t n)
{
	...
	chip->ecc_buf_cfg = boot_layout ? 0x1ff : 0x203;
	chip->ecc_bch_cfg = (chip->ecc_bch_cfg & ~ECC_NUM_DATA_BYTES_MASK)
...
no_dual_nand_ctlr_support:
	...
	nand_mtd_for_flash_chardev = &info->mtd;
...
static int __devexit msm_nand_remove(struct platform_device *pdev)
{
	...
	nand_mtd_for_flash_chardev = NULL;
