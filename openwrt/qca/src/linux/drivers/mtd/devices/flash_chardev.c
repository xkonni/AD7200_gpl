...
#define FLASH_SECTOR_SIZE       (mtd->erasesize) //(64 * 1024)
...
tp_flash_ioctl(struct file *file,  unsigned int cmd, unsigned long arg)
{
    ...
    extern struct mtd_info *nand_mtd_for_flash_chardev;
    struct mtd_info *mtd = NULL;

    if (!mtd_for_flash_chardev && !nand_mtd_for_flash_chardev) {
        printk("flash_device_mtd and nand_flash_device_mtd are NULL!\n");
        goto wrong;
    } else {
        mtd = nand_mtd_for_flash_chardev ? nand_mtd_for_flash_chardev : mtd_for_flash_chardev;
    }
    rwBuf = (unsigned char *)kmalloc(mtd->erasesize * 2, GFP_KERNEL);
